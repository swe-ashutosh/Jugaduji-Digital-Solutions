...
import { processDocument, cleanTempFile } from './services/local-processor'
...
// --- CSC Solutions Document Processing APIs ---
...
app.post('/api/process-document', async (c) => {
  try {
    ...
    const result = await processDocument(fileBuffer, mimeType, options)
    
    if (result.status === 'auth_required') {
      return c.json({ status: 'auth_required' })
    }

    if (result.status === 'success') {
      return c.json({
        status: 'success',
        downloadUrl: `http://localhost:8787/api/temp/${result.filename}`,
    ...
    console.error('[API] Error in process-document:', err)
    return c.json({ error: err.message || String(err) }, 500)
  }
})

// 2. Stream Temp File and Delete Immediately After Stream Ends
app.get('/api/temp/:filename', async (c) => {
  const filename = c.req.param('filename')
  const filePath = path.join(TEMP_DIR, filename)

  if (!fs.existsSync(filePath)) {
    return c.text('File not found or already deleted', 404)
  }
  ...
  return stream(c, async (stream) => {
    ...
    fileStream.close()
    // Small buffer delay to release file descriptor before unlink
    setTimeout(() => {
      cleanTempFile(filePath)
    }, 1000)
  })
...
// 3. Print document trigger - deletes temp files immediately
app.post('/api/print-document', async (c) => {
  try {
    ...
    const filePath = path.join(TEMP_DIR, filename)
    if (fs.existsSync(filePath)) {
      // Simulate sending to printing system
      console.log(`[API] Simulating print for document: ${filename}`)
      
      // Delete immediately
      cleanTempFile(filePath)
      return c.json({ status: 'success', message: 'Print triggered and file deleted' })
    }

    return c.json({ error: 'File not found or already cleaned up' }, 404)
  } catch (err: any) {
    return c.json({ error: err.message || String(err) }, 500)
  }
})
